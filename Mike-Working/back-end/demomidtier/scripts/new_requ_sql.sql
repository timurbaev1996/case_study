...
*/
SELECT i.instrument_name, AVG(d.deal_amount) AS avg_sell_price
  FROM deal d, instrument i
  WHERE d.deal_type = 'S' 
...
INNER JOIN instrument i on  d.deal_instrument_id = i.instrument_id
  Group by i.instrument_name, c.counterparty_name;  
/*
select  c.counterparty_name, i.instrument_name, SUM(case when d.deal_type = 'S' then d.deal_amount else 0 end) - SUM(case when d.deal_type = 'B' then d.deal_amount else 0 end) as net_sum
FROM  deal d
INNER JOIN counterparty c on d.deal_counterparty_id = c.counterparty_id 
INNER JOIN instrument i on  d.deal_instrument_id = i.instrument_id
  Group by i.instrument_name, c.counterparty_name;  
